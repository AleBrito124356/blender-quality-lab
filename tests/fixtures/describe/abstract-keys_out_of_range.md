...
Blender 5.2.1 LTS, CYCLES 960 x 720. Gates: **9/10 passed**, 1 warnings.

## Fix first

1. **'Copper orbit 3' is keyed on frames 300-400, outside 1-250** Make the frame range cover the keys (or move the keys into the range); delete non-finite keys.
   `bpy.context.scene.frame_start, bpy.context.scene.frame_end = 300, 400`
2. **Key 'Cool fill' and fill 'Large warm key' are nearly equal on the visible subject (1.06:1); fine for catalogue shots, raise the key for more shape** For more shape, double 'Cool fill' or halve 'Large warm key'.
   `bpy.data.objects['Cool fill'].data.energy = 800`

## Failed gates

- `animation_keys_in_range`: 'Copper orbit 3' is keyed on frames 300-400, outside 1-250
...
Frames 1-250 at 24.0 fps.
- 'Copper orbit 3': rotation_euler keyed on frames [300.0, 400.0].
Lowest share of the subject in frame over the animation: 100.0%.
